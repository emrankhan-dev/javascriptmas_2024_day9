...
const suitableRecipes = recipes.filter((recipe) => {
  const hasLiked = recipe.ingredients.some(ingredient => guest.loves.includes(ingredient))
  const hasDisLiked = recipe.ingredients.some(ingredient => guest.dislikes.includes(ingredient))
  return hasLiked && !hasDisLiked
})

const finalRecipeOfLikes = suitableRecipes.map(recipe => recipe.name)

// Step 2: Output the suitable recipes
document.getElementById('recipe').innerHTML = `<p>Based on the ${guest.name}'s preferences she likes: </p><h4>🍽️ ${finalRecipeOfLikes.join(" & ")} 🍽️<h4>`
